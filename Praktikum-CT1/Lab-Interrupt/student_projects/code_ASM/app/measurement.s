...
init_exti       ; Clock configuration
                ...
                LDR  R7, =0x4000            ; Enable SYSCFG clock
                BL   set_sfr
                
                ; Input pin configuration (PA.0)
                LDR  R6, =REG_GPIOA_PUPDR
                LDR  R7, =0x2               ; Set pin to pull-down mode
                BL   set_sfr
                
                ; Configure EXTI
                LDR  R6, =REG_EXTI_IMR
                LDR  R7, =0x1               ; Unmask EXTI0 interrupt
                BL   set_sfr
                
                LDR  R6, =REG_EXTI_RTSR
                LDR  R7, =0x1               ; Trigger on rising edge
                BL   set_sfr
                
                    
init_timer      ; Clock configuration
                LDR  R6, =REG_RCC_APB1ENR
                LDR  R7, =0x1               ; Enable TIM2 clock
                BL   set_sfr
                
                ; Configure TIM2 frequency
                LDR  R6, =REG_TIM2_PSC
                LDR  R7, =839               ; Timer prescaler => 84Mhz / 840 = 100kHz => 10us
                BL   set_sfr
                
                LDR  R6, =REG_TIM2_ARR
                LDR  R7, =0xffffffff        ; Clear Register first, it gets
                BL   clear_sfr              ;   initialized with 0xffffffff
                LDR  R7, =199998             ; Autoreload value => 99999+1 => 1s
                BL   set_sfr
                
                LDR  R6, =REG_TIM2_EGR
                LDR  R7, =0x1               ; Update TIM2 configuration
                BL   set_sfr
                
                LDR  R6, =REG_TIM2_DIER
                LDR  R7, =0x1               ; Enable interrupt source
                BL   set_sfr
                
                LDR  R6, =REG_TIM2_CR1
                LDR  R7, =0x1               ; Enable TIM2
                BL   set_sfr
...
clear_IRQ_EXTI0 PROC
                ...
                PUSH {R6-R7, LR}
                LDR  R6, =REG_EXTI_PR
                LDR  R7, =0x1               ; Clear irq pending bit
                BL   set_sfr
                POP  {R6-R7, PC}
                ENDP
