...
{
    /// STUDENTS: To be programmed
		uint16_t adc_value, normalized_value;
		...
		while(1)
		{
			...
			normalized_value = normalize_value(adc_value, resolution);
			display_on_lcd(normalized_value);
		}
...
static uint16_t normalize_value(uint16_t value, adc_resolution_t resolution)
{
    ...
    /// STUDENTS: To be programmed
		...
		normalized = value* 3300 / ((1<<bit_width) - 1);
    /// END: To be programmed
